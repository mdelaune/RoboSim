...
void Vacuum::move(const QList<QRectF>& rooms, const QList<Obstruction2>& obstructions, const QList<QPointF>& doors, int multiplier)
{
    ...
    // Find which room we're currently in
    ...
    {
        // Move vacuum based on pathing algorithm
        QPointF currentPos = PathAlgorithms::moveRandomly(currentPosition, velocity, speed * multiplier, currentRoom, obstructions, doors);
        vacuumGraphic->setPos(currentPos);
        qDebug() << "Moved to:" << currentPos;
    }
